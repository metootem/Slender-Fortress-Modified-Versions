...
#define HIDEHUD_SF2_DEFAULT (HIDEHUD_CROSSHAIR | HIDEHUD_HEALTH | HIDEHUD_CLOAK_AND_FEIGN | HIDEHUD_PIPES_AND_CHARGE | HIDEHUD_BUILDING_STATUS | HIDEHUD_METAL | HIDEHUD_MATCH_STATUS)
...
bool SF_IsSurvivalInvertedMap()
{
	return (g_IsSurvivalInvertedMap || (g_SurvivalInvertedMapConVar.IntValue == 1));
}
...
bool TraceRayDontHitEntity(int entity, int mask, any data)
{
	...
	int collisionGroup = GetEntProp(entity, Prop_Send, "m_CollisionGroup");
	if (collisionGroup == 1)
	{
		return false;
	}
...
int SF_CurrentSpecialRound(int specialRoundList[SPECIALROUND_MAXROUNDS])
{
	if (!g_IsSpecialRound)
	{
		return 0;
	}
	int specialRoundCount = 0;
	for (int array = 0; array < SPECIALROUND_MAXROUNDS; array++)
	{
		if (g_ArraySpecialRoundType[array] != 0)
		{
			specialRoundList[specialRoundCount] = g_ArraySpecialRoundType[array];
			specialRoundCount++;
		}
	}
	return specialRoundCount;
}
