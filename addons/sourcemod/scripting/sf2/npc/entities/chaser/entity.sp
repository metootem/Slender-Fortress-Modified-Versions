...
methodmap SF2_ChaserEntity < SF2_BaseBoss
{
	...
	public bool CanBeStunned()
	{
		...
		if (!this.Controller.GetProfileData().StunData.Enabled[this.Controller.Difficulty] && !g_SlenderForceStunnable[this.Controller.Index])
		{
			return false;
		}
...
static void ProcessSpeed(SF2_ChaserEntity chaser)
{
	...
	float speed, acceleration, yawRate = originalData.TurnRate;
	...
	switch (moveType)
	{
		...
		case SF2NPCMoveType_Attack:
		{
			...
			yawRate = attackData.RunYawRate[difficulty];
		}
	}
...
static bool LocoCollideWith(CBaseNPC_Locomotion loco, int other)
{
	...
	if (IsValidEntity(other))
	{
		...
		if (player.IsValid)
		{
			...
			if (!player.CanCollideWithBosses)
			{
				return false;
			}
