...
void SetupNPCGlows()
{
	...
	g_OnSlenderGlowForcePFwd.AddFunction(null, OnSlenderGlowForce);
}
...
static void OnSlenderGlowForce(SF2NPC_BaseNPC controller)
{
	UpdateVisibility(controller);
}
...
static bool ShouldBeVisibleToPlayer(SF2NPC_BaseNPC controller, SF2_BasePlayer other)
{
	...
	if (!other.IsEliminated)
	{
		...
		return SF_SpecialRound(SPECIALROUND_WALLHAX) || g_RenevantWallHax || g_EnableWallHaxConVar.BoolValue || g_SlenderForceGlowVisible[controller.Index][other.index];
	}
