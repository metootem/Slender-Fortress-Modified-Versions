	...
	for (int i = 0; i < MAX_BOSSES; i++)
	{
		...
		else if (!IsClientInDeathCam(client))
		{
			if (boss && boss != INVALID_ENT_REFERENCE)
			{
				...
				if (g_PlayerStaticAmount[client] >= 1.0 && !g_StaticDisabled)
				{
...
void ClientPerformScare(int client, int bossIndex)
{
	...
	if (!g_StaticDisabled && !g_PlayerStaticForceDisabled[client])
	{
...
{
	if (GetGameTime() < g_PlayerStressTimeAllow[client])
	{
		return;
	}

	g_PlayerStressTimeAllow[client] = GetGameTime() + 0.3;
...
void ClientResetOverlay(int client)
{
	...
	g_PlayerForcedOverlay[client][0] = 0;
...
Action Timer_PlayerOverlayCheck(Handle timer, any userid)
{
	...
	//LogMessage(g_PlayerForcedOverlay[client]);
	if (g_PlayerForcedOverlay[client][0] && !g_PlayerEliminated[client] && !IsClientInDeathCam(client))
	{
		strcopy(material, sizeof(material), g_PlayerForcedOverlay[client]);
	}
	else if (IsClientInDeathCam(client) && deathCamBoss != -1 && g_PlayerDeathCamShowOverlay[client])
	{
...
void ClientShowTimerHud(int client, int color[4] = {SF2_HUD_TEXT_COLOR_R, SF2_HUD_TEXT_COLOR_G, SF2_HUD_TEXT_COLOR_B, SF2_HUD_TEXT_COLOR_A}, 
									char[] Msg="")
{
	// 0.06
	SetHudTextParams(-1.0, 0.12,
		1.0,
	...
	ShowSyncHudText(client, g_RoundTimerSync, Msg);
}
