...
void ClientSetPluginWalkSpeed(int client, float value)
{
	if (value < 0.0)
		g_PlayerPluginWalkSpeed[client] = 0.0;
	else
		g_PlayerPluginWalkSpeed[client] = value;
}

float ClientAddPluginWalkSpeed(int client, float value)
{
	if (!IsValidClient(client))
		return 0.0;
	
	g_PlayerPluginWalkSpeed[client] += value;
	return g_PlayerPluginWalkSpeed[client];
}

float ClientSubtractPluginWalkSpeed(int client, float value)
{
	if (!IsValidClient(client))
		return 0.0;
	
	g_PlayerPluginWalkSpeed[client] -= value;
	if (g_PlayerPluginWalkSpeed[client] < 0.0)
		g_PlayerPluginWalkSpeed[client] = 0.0;
	return g_PlayerPluginWalkSpeed[client];
}

float ClientSetPluginSprintSpeed(int client, float value)
{
	if (!IsValidClient(client))
		return 0.0;
	...
	if (value < 0.0)
		...
		g_PlayerPluginSprintSpeed[client] = value;
	return g_PlayerPluginSprintSpeed[client];
}

float ClientAddPluginSprintSpeed(int client, float value)
{
	if (!IsValidClient(client))
		return 0.0;
...
float ClientSubtractPluginSprintSpeed(int client, float value)
{
	if (!IsValidClient(client))
		return 0.0;
	
	g_PlayerPluginSprintSpeed[client] -= value;
	if (g_PlayerPluginSprintSpeed[client] < 0.0)
		g_PlayerPluginSprintSpeed[client] = 0.0;
	return g_PlayerPluginSprintSpeed[client];
}
...
static void Hook_SpeedThink(int client)
{
	...
	if (player.PluginWalkSpeed)
		walkSpeed += player.PluginWalkSpeed;
	if (player.PluginSprintSpeed)
		sprintSpeed += player.PluginSprintSpeed;
	...
	if (player.IsSprinting)
	{
		if (!SF_SpecialRound(SPECIALROUND_RUNNINGINTHE90S) && !g_Renevant90sEffect)
		{
			...
			player.SetPropFloat(Prop_Send, "m_flCurrentTauntMoveSpeed", sprintSpeed);
		}
		else
		{
			player.SetPropFloat(Prop_Send, "m_flMaxspeed", sprintSpeed > 520.0 ? sprintSpeed : 520.0);
			player.SetPropFloat(Prop_Send, "m_flCurrentTauntMoveSpeed", sprintSpeed);
		}
	}
	else
...
{
	float newValue = float(GetNativeCell(2)) / 100.0;
