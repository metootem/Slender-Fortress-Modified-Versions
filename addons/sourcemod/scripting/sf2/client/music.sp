	...
	if (MusicActive())
	{
		for (int i = 1; i <= MaxClients; i++)
		{
			...
			if (path[0] != '\0')
			{
				...
				EmitSoundToClient(i, path, SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_STOP);
			}
...
void ClientUpdateMusicSystem(int client, bool initialize = false)
{
	...
	if (IsRoundEnding() || !IsValidClient(client) || IsFakeClient(client) || DidClientEscape(client) || (g_PlayerEliminated[client] && !IsClientInGhostMode(client) && !g_PlayerProxy[client]) || SF_SpecialRound(SPECIALROUND_BEATBOX))
	{
		...
		if (MusicActive()) //A boss is overriding the music.
		{
			...
			if (path[0] != '\0')
			{
				...
				EmitSoundToClient(client, path, SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_STOP);
			}
		}
	}
	else
	...
	if (IsValidClient(client) && IsPlayerAlive(client))
	{
		...
		{
			if (MusicActive() && !g_DisallowBossMusic.BoolValue) //A boss is overriding the music.
			{
				GetBossMusic(path, sizeof(path));
		...
		// Custom system.
		if (!g_DisallowPageMusic.BoolValue)
		{
			...
			else
			{
				...
				if ((initialize || ClientHasMusicFlag2(oldMusicFlags, MUSICF_PAGES1PERCENT)) && !ClientHasMusicFlag(client, MUSICF_PAGES1PERCENT))
				{
					...
					EmitSoundToClient(client, MUSIC_GOTPAGES1_SOUND, SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_STOP);
				}
				...
				if ((initialize || ClientHasMusicFlag2(oldMusicFlags, MUSICF_PAGES25PERCENT)) && !ClientHasMusicFlag(client, MUSICF_PAGES25PERCENT))
				{
					...
					EmitSoundToClient(client, MUSIC_GOTPAGES2_SOUND, SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_STOP);
				}
				...
				if ((initialize || ClientHasMusicFlag2(oldMusicFlags, MUSICF_PAGES50PERCENT)) && !ClientHasMusicFlag(client, MUSICF_PAGES50PERCENT))
				{
					...
					EmitSoundToClient(client, MUSIC_GOTPAGES3_SOUND, SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_STOP);
				}
				...
				if ((initialize || ClientHasMusicFlag2(oldMusicFlags, MUSICF_PAGES75PERCENT)) && !ClientHasMusicFlag(client, MUSICF_PAGES75PERCENT))
				{
					...
					EmitSoundToClient(client, MUSIC_GOTPAGES4_SOUND, SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_STOP);
				}
		...
		if (!g_DisallowBossMusic.BoolValue)
		{
...
void ClientMusicReset(int client)
{
	...
	if (IsValidClient(client) && oldMusic[0] != '\0')
	{
		...
		EmitSoundToClient(client, oldMusic, SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_STOP);
	}
	...
	for (int i = 0; i < MAX_BOSSES; i++)
	{
		...
		if (NPCGetUniqueID(i) != -1 && NPCGetType(i) == SF2BossType_Chaser)
		{
			if (IsValidClient(client))
			{
				...
				for (int i2; i2 < soundInfo.Paths.Length; i2++)
				{
					char buffer[PLATFORM_MAX_PATH];
					soundInfo.Paths.GetString(i2, buffer, sizeof(buffer));
					EmitSoundToClient(client, buffer, SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_STOP);
				}
			}
		}
	}

	Call_StartForward(g_OnClientMusicStop);
	Call_PushCell(client);
	Call_PushCell(-2);
	Call_PushCell(STATE_ALERT);
	Call_Finish();
}
...
void Client90sMusicReset(int client)
{
	...
	if (IsValidClient(client))
	{
		...
		if (oldMusic[0] != '\0')
		{
			...
			Call_PushCell(client);
			Call_PushCell(-1);
			Call_PushCell(SPECIALROUND_RUNNINGINTHE90S);
			Call_Finish();
		}
	}
}
...
Action Timer_PlayerFadeInMusic(Handle timer, any userid)
{
	...
	{
		EmitSoundToClient(client, g_PlayerMusicString[client], SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_CHANGEVOL, g_PlayerMusicVolume[client] * g_PlayerPreferences[client].PlayerPreference_MusicVolume, 100);
	}
...
Action Timer_PlayerFadeInAlertMusic(Handle timer, any userid)
{
	...
	{
		EmitSoundToClient(client, g_PlayerAlertMusicString[client][bossIndex], SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_CHANGEVOL, g_PlayerAlertMusicVolumes[client][bossIndex] * info.Volume * g_PlayerPreferences[client].PlayerPreference_MusicVolume, info.Pitch);
	}
	...
	if (g_PlayerAlertMusicVolumes[client][bossIndex] >= 1.0)
	{
		...
		Call_StartForward(g_OnClientMusicStart);
		Call_PushCell(client);
...
Action Timer_PlayerFadeInIdleMusic(Handle timer, any userid)
{
	...
	{
		EmitSoundToClient(client, g_PlayerIdleMusicString[client][bossIndex], SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_CHANGEVOL, g_PlayerIdleMusicVolumes[client][bossIndex] * info.Volume * g_PlayerPreferences[client].PlayerPreference_MusicVolume, info.Pitch);
	}
...
Action Timer_PlayerFadeInChaseMusic(Handle timer, any userid)
{
	...
	{
		EmitSoundToClient(client, g_PlayerChaseMusicString[client][bossIndex], SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_CHANGEVOL, g_PlayerChaseMusicVolumes[client][bossIndex] * info.Volume * g_PlayerPreferences[client].PlayerPreference_MusicVolume, info.Pitch);
	}
...
Action Timer_PlayerFadeInChaseMusicSee(Handle timer, any userid)
{
	...
	{
		EmitSoundToClient(client, g_PlayerChaseMusicSeeString[client][bossIndex], SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_CHANGEVOL, g_PlayerChaseMusicSeeVolumes[client][bossIndex] * info.Volume * g_PlayerPreferences[client].PlayerPreference_MusicVolume, info.Pitch);
	}
...
Action Timer_PlayerFadeOutChaseMusic(Handle timer, any userid)
{
	...
	if (g_PlayerChaseMusicVolumes[client][bossIndex] <= 0.0)
	{
		if (info.Restart)
		{
			EmitSoundToClient(client, g_PlayerChaseMusicString[client][bossIndex], SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_STOP);
		}
		g_PlayerChaseMusicTimer[client][bossIndex] = null;
...
Action Timer_PlayerFadeIn90sMusic(Handle timer, any userid)
{
	...
	{
		EmitSoundToClient(client, g_Player90sMusicString[client], SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_CHANGEVOL, g_Player90sMusicVolumes[client] * g_PlayerPreferences[client].PlayerPreference_MusicVolume, 100);
	}
...
Action Timer_PlayerFadeOut90sMusic(Handle timer, any userid)
{
	...
	if (buffer[0] != '\0')
	{
		EmitSoundToClient(client, buffer, SOUND_FROM_LOCAL_PLAYER, MUSIC_CHAN, SNDLEVEL_NONE, SND_CHANGEVOL, g_Player90sMusicVolumes[client] * g_PlayerPreferences[client].PlayerPreference_MusicVolume, 100);
	}
