...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	g_OnClientPreKillDeathCamFwd = new GlobalForward("SF2_OnClientPreKillDeathCam", ET_Hook, Param_Cell, Param_Cell);
	...
	g_OnClientPreInstantKillFwd = new GlobalForward("SF2_OnClientPreInstantKill", ET_Hook, Param_Cell, Param_Cell);
	g_OnClientAntiCampingStrikeFwd = new GlobalForward("SF2_OnClientAntiCampingStrike", ET_Hook, Param_Cell, Param_Cell);
	g_OnSlenderVisionTraceEntityFwd = new GlobalForward("SF2_OnSlenderVisionTraceEntity", ET_Hook, Param_Cell, Param_Cell);
	g_OnSlenderAttackTraceEntityFwd = new GlobalForward("SF2_OnSlenderAttackTraceEntity", ET_Hook, Param_Cell, Param_Cell);
	g_OnRoundTimeCountFwd = new GlobalForward("SF2_OnRoundTimeCount", ET_Ignore, Param_Cell);
	g_OnEscapeTimeCountFwd = new GlobalForward("SF2_OnEscapeTimeCount", ET_Ignore, Param_Cell);
	g_OnClientMusicStart = new GlobalForward("SF2_OnClientMusicStart", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);
	g_OnClientMusicStop = new GlobalForward("SF2_OnClientMusicStop", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);
	...
	CreateNative("SF2_GetRoundTimeCount", Native_GetRoundTime);
	...
	CreateNative("SF2_SetCustomDifficultyString", Native_SetCustomDifficultyString);
	...
	CreateNative("SF2_GetMaxPages", Native_GetMaxPages);
	CreateNative("SF2_RevealPages", Native_RevealPages);
	...
	CreateNative("SF2_SetBossStunnable", Native_SetBossStunnable);
	CreateNative("SF2_GetBossForceStunnable", Native_GetBossForceStunnable);
	CreateNative("SF2_SetBossStunDisappear", Native_SetBossStunDisappear);
	CreateNative("SF2_GetBossForceStunDisappear", Native_GetBossForceStunDisappear);
	CreateNative("SF2_SetBossForceGlow", Native_SetBossForceGlow);
	CreateNative("SF2_GetBossForceGlow", Native_GetBossForceGlow);
	CreateNative("SF2_SetBossTeleportMaxTargeted", Native_SetBossTeleportMaxTargeted);
	CreateNative("SF2_SetBossIgnoreTargeted", Native_SetBossIgnoreTargeted);
	...
	CreateNative("SF2_SetClientWalkSpeed", Native_SetClientWalkSpeed);
	CreateNative("SF2_SetClientSprintSpeed", Native_SetClientSprintSpeed);

	CreateNative("SF2_SetStaticEnabled", Native_SetStaticEnabled);

	CreateNative("SF2_IsClientMusicPlaying", Native_IsClientMusicPlaying);
	CreateNative("SF2_GetClientUltravisionEnt", Native_GetClientUltravisionEnt);
...
static any Native_GetRoundTime(Handle plugin, int numParams)
{
	return g_RoundTime;
}
...
static any Native_SetCustomDifficultyString(Handle plugin, int numParams)
{
	GetNativeString(1, g_CustomDifficultyString, sizeof(g_CustomDifficultyString));
	return 0;
}
...
static any Native_GetMaxPages(Handle plugin, int numParams)
{
	return g_PageMax;
}

static any Native_RevealPages(Handle plugin, int numParams)
{
	bool reveal = GetNativeCell(1);
	bool empty = GetNativeCell(2);
	int color[4], color2[4];
	GetNativeArray(3, color, 4);
	GetNativeArray(4, color2, 4);
	return RevealPages(reveal, empty, color, color2);
}
...
static any Native_SetBossStunnable(Handle plugin, int numParams)
{
	g_SlenderForceStunnable[GetNativeCell(1)] = GetNativeCell(2);
	return 0;
}

static any Native_GetBossForceStunnable(Handle plugin, int numParams)
{
	return g_SlenderForceStunnable[GetNativeCell(1)];
}

static any Native_SetBossStunDisappear(Handle plugin, int numParams)
{
	g_SlenderForceStunDisappear[GetNativeCell(1)] = GetNativeCell(2);
	return 0;
}

static any Native_GetBossForceStunDisappear(Handle plugin, int numParams)
{
	return g_SlenderForceStunDisappear[GetNativeCell(1)];
}

static any Native_SetBossTeleportMaxTargeted(Handle plugin, int numParams)
{
	...
	controller.SetTeleportMaxTargeted(GetNativeCell(2), GetNativeCell(3));
	return 0;
}

static any Native_SetBossIgnoreTargeted(Handle plugin, int numParams)
{
	SF2NPC_BaseNPC controller = SF2NPC_BaseNPC(GetNativeCell(1));
	if (!controller.IsValid())
	{
	...
	controller.SetTeleportIgnoreTargeted(GetNativeCell(2), GetNativeCell(3));
	return 0;
}

static any Native_SetBossForceGlow(Handle plugin, int numParams)
{
	int bossIndex = GetNativeCell(1);
	int clients[MAXTF2PLAYERS];
	GetNativeArray(2, clients, MAXTF2PLAYERS);
	int maxclients = GetNativeCell(3);
	bool state = GetNativeCell(4);
	for (int i; i < maxclients; i++)
	{
		g_SlenderForceGlowVisible[bossIndex][clients[i]] = state;
	}
	
	Call_StartForward(g_OnSlenderGlowForcePFwd);
	Call_PushCell(SF2NPC_BaseNPC(bossIndex));
	Call_Finish();

	return 0;
}

static any Native_GetBossForceGlow(Handle plugin, int numParams)
{
	return g_SlenderForceGlowVisible[GetNativeCell(1)][GetNativeCell(2)];
}
...
static any Native_SetClientWalkSpeed(Handle plugin, int numParams)
{
	return ClientSetPluginWalkSpeed(GetNativeCell(1), GetNativeCell(2));
}

static any Native_SetClientSprintSpeed(Handle plugin, int numParams)
{
	return ClientSetPluginSprintSpeed(GetNativeCell(1), GetNativeCell(2));
}

static any Native_SetStaticEnabled(Handle plugin, int numParams)
{
	g_StaticDisabled = !GetNativeCell(1);
	return g_StaticDisabled;
}

static any Native_IsClientMusicPlaying(Handle plugin, int numParams)
{
	int client = GetNativeCell(1);
	for (int bossIndex; bossIndex < MAX_BOSSES; bossIndex++)
	{
		if (g_PlayerChaseMusicString[client][bossIndex][0] != '\0' ||
			...
			g_Player90sMusicString[client][bossIndex][0] != '\0')
			return true;
	}
	return false;
}

static any Native_GetClientUltravisionEnt(Handle plugin, int numParams)
{
	return EntRefToEntIndex(g_PlayerUltravisionEnt[GetNativeCell(1)]);
}