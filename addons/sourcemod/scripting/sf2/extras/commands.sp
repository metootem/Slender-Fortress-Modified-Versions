...
public void OnPluginStart()
{
	...
	g_RenevantWaveTime = CreateConVar("sf2_renevant_wave_time", "60", "How often in seconds should a wave be triggered.", _, true, 0.0);
	...
	g_SurvivalInvertedMapConVar = CreateConVar("sf2_issurvivalinvertedmap", "0", "Set to 1 if the map is using survival inverted map.", _, true, 0.0, true, 1.0);
	...
	g_DisallowBossMusic = CreateConVar("sf2_disallow_boss_music", "0", "Blocks bosses from playing their background music (not chase theme, alert, etc.).", _, false, 0.0, true, 1.0);
	g_DisallowBossMusic.AddChangeHook(OnConVarChanged);
	g_LowTimePingTime = CreateConVar("sf2_low_time_ping_time", "0.0", "Time under which sf2_low_time_ping_sound works.", _, true, 0.0);
	g_LowTimePingSound = CreateConVar("sf2_low_time_ping_sound", "", "Sound that plays every second under sf2_low_time_ping_time.");

	g_DisallowPageMusic = CreateConVar("sf2_disallow_page_music", "0", "Prevents map page music from playing.", _, true, 0.0, true, 1.0);
	g_DisallowPageMusic.AddChangeHook(OnConVarChanged);
	...
	g_OnSlenderGlowForcePFwd = new PrivateForward(ET_Ignore, Param_Cell);
...
static Action Command_RevealPageLocations(int client, int args)
{
	...
	int count = RevealPages(!g_PagesRevealed);
	if (g_PagesRevealed)
	{
