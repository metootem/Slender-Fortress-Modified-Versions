...
bool LoadBossProfile(KeyValues kv, const char[] profile, char[] loadFailReasonBuffer, int loadFailReasonBufferLen, bool lookIntoLoads = false, const char[] originalDir = "")
{
	...
	GetProfileDifficultyNumValues(kv, "teleport_target_max_targeted", profileData.TeleportMaxTargeted, profileData.TeleportMaxTargeted);
	GetProfileDifficultyBoolValues(kv, "teleport_target_ignore_targeted", profileData.TeleportIgnoreTargeted, profileData.TeleportIgnoreTargeted);
...
int GetBossProfileTeleportMaxTargeted(const char[] profile, int difficulty)
{
	g_BossProfileData.GetArray(profile, g_CachedProfileData, sizeof(g_CachedProfileData));
	return g_CachedProfileData.TeleportMaxTargeted[difficulty];
}

bool GetBossProfileTeleportIgnoreTargeted(const char[] profile, int difficulty)
{
	g_BossProfileData.GetArray(profile, g_CachedProfileData, sizeof(g_CachedProfileData));
	return g_CachedProfileData.TeleportIgnoreTargeted[difficulty];
}
